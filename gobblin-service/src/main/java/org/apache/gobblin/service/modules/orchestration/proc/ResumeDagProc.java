...
public final class ResumeDagProc extends DagProc {
  ...
  protected void act() throws IOException {
    DagActionStore.DagAction dagAction = new DagActionStore.DagAction(dagIdToResume.getFlowGroup(),
        dagIdToResume.getFlowName(), dagIdToResume.getFlowExecutionId(), DagActionStore.FlowActionType.RESUME);
    ...
    if (this.dagToResume == null || !this.dagManager.getFailedDagIds().contains(dagToResume.toString())) {
      ...
      this.dagManager.removeDagActionFromStore(dagAction);
      return;
    }
